...
# Page Configuration
st.set_page_config(
    page_title="DataScout",
    page_icon="🧊",  # Subtle ice cube or chart icon
    layout="wide",
...
# Load Google Fonts
st.markdown('''
<link href="https://fonts.googleapis.com/css2?family=Inter:wght@400;600;700&family=Source+Sans+Pro:wght@400;600&family=JetBrains+Mono&display=swap" rel="stylesheet">
...
def main():
    # --- Custom Header ---
    st.markdown('''
    <div style="text-align: left; padding-bottom: 5px; color: #6c757d; font-size: 0.85rem; font-family: 'Source Sans Pro', sans-serif;">
        Made for FWD 2026 by Illia Shybanov
    </div>
    ''', unsafe_allow_html=True)

    st.markdown('''
    <h1 style="color: #1e3a5f; font-family: 'Inter', sans-serif; font-weight: 700; margin-bottom: 0;">
        DataScout
    </h1>
    <p style="color: #6c757d; margin-top: 5px; font-size: 1.1rem;">
        AI-Powered Dataset Intelligence Platform
    </p>
    <hr style="margin-top: 5px; margin-bottom: 25px; border-top: 1px solid #e0e0e0;">
    ''', unsafe_allow_html=True)
    ...
    with tab3:
        ...
        st.metric("Total Anomalies", summary['total_anomalies'], delta=f"{summary['anomaly_rate']:.1%} Rate", delta_color="inverse")
        ...
        if not flagged_df.empty:
            ...
            st.dataframe(
                flagged_df[cols].style.apply(lambda x: ['background-color: #fff5f5' for i in x], axis=1),
    ...
    with tab4:
        ...
        if st.checkbox("Show Context (Debug)"):
